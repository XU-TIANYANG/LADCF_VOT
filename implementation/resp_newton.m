function [disp_row, disp_col, sind] = resp_newton(response1, response2, responsef1, responsef2, iterations, ky, kx, use_sz)
if response2 ~= 0
    sz = size(kx,1);
    ...
    weight = 0.005:0.005:0.995;
    L = zeros(numel(weight),1);
    i = floor((size(response1,3)+1)/2);
    r1 = response1(:,:,i);
    r2 = response2(:,:,i);
